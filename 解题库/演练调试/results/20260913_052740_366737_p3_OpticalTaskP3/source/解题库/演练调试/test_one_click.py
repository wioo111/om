...
def isolated_launcher_lock(tmp_path,monkeypatch):
    # Exercise the real OS lock in an isolated directory. Never contend with a
    # user's active one-click robot or accept an unrelated lock error as a guard pass.
    monkeypatch.setattr(launch,'ROOT',tmp_path/'isolated-launcher')
...
def test_formal_session_never_dispatches_robot(tmp_path,monkeypatch):
    ...
    record=json.loads(next((tmp_path/'out').glob('*/execution.json')).read_text(encoding='utf-8'))
    assert 'NOT A VERIFIED PRACTICE' in record['error']


@pytest.mark.skipif(os.name!='nt',reason='Windows one-click lock')
def test_dispatch_refuses_a_held_lock(tmp_path):
    def forbidden(args):pytest.fail('Robot must not dispatch when another robot holds the lock')
    with launch.one_robot(launch.ROOT/'.practice.lock'):
        assert launch.execute(3,tmp_path/'unused.json',tmp_path,tmp_path/'out',runner=forbidden)==1
    record=json.loads(next((tmp_path/'out').glob('*/execution.json')).read_text(encoding='utf-8'))
    assert '已有一个接入程序' in record['error']
