"""P3 practice candidates: observation-only residual coverage and en-route sensing."""
...
class ResidualP3(HopP3):
    ...
    def on_measure(self, state, ch, result, svd):
        ...
        if np.count_nonzero(remaining) > len(remaining) * 0.35:
            return
        ...
        self.residual_updates += int(np.count_nonzero(remaining) < before)
