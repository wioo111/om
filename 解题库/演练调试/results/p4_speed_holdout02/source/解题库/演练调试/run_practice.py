...
class PracticeGuard:
    def __init__(self,data):
        ...
        if len(active)!=1:raise RuntimeError('Require exactly one active simulator journal')
        self.journal=active[0]
        self.header=self.read_header()
        self.check()
        events=[json.loads(s) for s in self.journal.read_text(encoding='utf-8').splitlines() if s.strip()]
        if not any(e.get('event')=='api_opened' for e in events):raise RuntimeError('Practice API is not ready')
        if any(e.get('entered') for e in events):raise RuntimeError('Another robot has already entered this practice')

    def read_header(self):
        with self.journal.open(encoding='utf-8') as f:return json.loads(f.readline())
...
def main():
    ...
    p.add_argument('--case',required=True,help='Case code observed on the practice page; never used by strategy')
    ...
    metadata=dict(mode='practice',problem=args.problem,case=args.case,strategy=strategy.name,
                  ...
                  code_sha256=hashes,command=['run_practice.py','--problem',str(args.problem),'--strategy',args.strategy,'--case',args.case,'--robot-id','<redacted>'])
    ...
    class PracticeClient(SimulatorClient):
        def _send(self,path,payload):
            ...
            try:
                ...
                if path=='/clear' or self._seq%100==0:
    ...
    try:
        ...
        matches=list((args.data/'behavior-logs').glob(f'practice-p{args.problem}-*-{args.case}.result.json'))
        if matches:
        ...
        with sqlite3.connect(db.resolve().as_uri()+'?mode=ro',uri=True) as con:
            ...
            row=con.execute('SELECT problem_no,case_code,end_reason,cleared_jammer_count,jammer_count,measure_accepted_count,virtual_time_us,clear_failure_count FROM practice_statistics_tasks WHERE case_code=? ORDER BY id DESC LIMIT 1',(args.case,)).fetchone()
            if row:result['simulator_summary']=dict(row)
