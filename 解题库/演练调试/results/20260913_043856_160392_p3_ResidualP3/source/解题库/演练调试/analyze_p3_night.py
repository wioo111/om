...
def main():
    ...
    parser.add_argument('--label', default='p3_night_analysis_'+datetime.now().strftime('%Y%m%d_%H%M%S'))
    args=parser.parse_args()
    day=date.fromisoformat(args.date)
    if Path(args.label).name != args.label or args.label in ('.', '..'):
        parser.error('label must be a new directory name')
    rows=[];inputs={}
    for folder in sorted((ROOT/'results').glob(day.strftime('%Y%m%d')+'_*_p3_*')):
    ...
    report=dict(created=datetime.now(timezone.utc).isoformat(),scope='completed local P3 practice records from '+day.isoformat(),
        input_sha256=inputs,summary=groups,records=rows)
    folder=ROOT/'results'/args.label;folder.mkdir(exist_ok=False)
