...
class P4MockSimulator:
    ...
    def _covered(self, src: dict, S) -> bool:
        ...
        # S 看到源的方位（从 S 指向 src），用于判断 src 能否"看到"S
        # 源看到的 S 方位 = 源指向 S 的方向 = atan2(-dx, -dy)
    ...
    def exit(self):
        return {'virtual_time_s': self.virtual_time_s,
    ...
    def measure(self, x: float, y: float, ch: int):
        ...
        if ch not in self.sources:
            ...
            if d > src['re']:
                ...
                result = 'direction'
                svd = (true_bearing + err) % 360
            else:
                # 在覆盖范围外
                result = 'no_signal'
                svd = None

        return {'measure_result': result, 'svd_deg': svd,
...
def _self_test():
    ...
    sim.enter()
    print("测向点 (0,0) 测频道 1：", sim.measure(0, 0, 1))
    sim.exit()
