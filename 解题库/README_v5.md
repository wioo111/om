...
> 本文件下方保留的是历史状态，不再具有入口或论文来源效力。当前正式入口见仓库根目录 `README.md`，唯一实现为 `第一二问/src/q12/`，唯一正文源为 `第一二问/paper/正文模板.md`。第一、二问旧版本不再作为正式计算和论文结果源。v5 脚本直接运行时已转接共享实现；第三、四问依赖的旧模块保持兼容。
