...
def run_case(spec):
    problem,variant,seed,n,mode,frac,folder=spec
    ...
    if problem==3:
        ...
        factory={'baseline':AdaptiveV8,'scan':FastScanP3,'cautious':CautiousP3,'compact':CompactP3}[variant]
        sim=MockSimulator(seed=seed,N=n,error_mode=mode)
    else:
...
def main():
    ...
    specs=[(args.problem,v,args.seed_start+i,10+i%7,'edge' if i%4==3 else 'fixed',
            (.25,.5,.75,1.)[i%4],str(traces)) for i in range(args.cases) for v in variants]
    rows=[]
    ...
    summary={}
    baseline={r['seed']:r for r in rows if r['variant']=='baseline'}
    for v in variants:
