...
from practice_launch import REPO, ROOT, Tee, one_robot, robot_id


def load_strategy(problem):
...
def execute(problem):
    module, strategy, client_type, post, run = load_strategy(problem)
                        ...
                        def _send(self, path, payload):
                            ...
                            try:
                                ...
                                if path in ('/enter', '/clear', '/exit') or self._seq % 100 == 0:
                                    print(json.dumps(dict(action=self._seq, path=path,
                                        virtual_time_s=response.get('virtual_time_s'),
                                        result=response.get('clear_result')), ensure_ascii=False), flush=True)
                                return response
...
def main(argv=None):
    ...
    if args.check:
        _, strategy, *_ = load_strategy(args.problem)
