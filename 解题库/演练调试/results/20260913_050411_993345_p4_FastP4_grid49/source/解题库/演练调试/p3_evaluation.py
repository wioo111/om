...
def benchmark(folder,args):
    ...
    plan=dict(created=utc_now(), stages='frozen offline validation, not official practice or a final blind test',
              ...
              profile_sha256=digest(dataset/'profile.json'), variants=VARIANTS, reference='residual',
    ...
    evaluation={model:stratified([r for r in rows if r['evaluation_model']==model],'residual') for model in models}
